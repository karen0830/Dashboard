...
export const addProduct = async (newProduct) => {
  try {
    console.log("dataEnvio", newProduct);
    const reader = new FileReader();
    // Eliminar el token del localStorage
    const data = {
      imgProduct: newProduct.images,
      info: newProduct
    }
    console.log(newProduct.images[0].file);
    const formData = new FormData();
    formData.append('imgProduct', newProduct.images[0].file);
    // Agregar el objeto newProduct como JSON al FormData
    formData.append('info', JSON.stringify(newProduct));
    const response = await ruta_protegida().post('/verifyProduct', formData, newProduct);
    return response;
  } catch (ex) {
    console.log("error.status:", ex);
  }
...
export const getOrders = async () => {
  try {
    const response = await ruta_protegida().get('/getOrders');
    return response;
  } catch (ex) {
    console.log("error.status:", ex);
  }
